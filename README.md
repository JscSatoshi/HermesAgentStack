...
浏览器
  │
  ├── https://127.0.0.1:9119 -> HTTPS Proxy -> Hermes Dashboard
  └── https://127.0.0.1:8787 -> HTTPS Proxy -> Hermes WebUI
                                 │
                                 ├── 共享 Hermes Home / Sessions / Memory
                                 ├── 共享 Hermes Agent 源码卷
                                 └── 共享 /workspace
                                               │
                                               ▼
...
Hermes WebUI 上游文档明确说明：多容器模式下，WebUI 触发的工具实际运行在 WebUI 容器里，而不是 gateway 容器里。如果直接使用官方 WebUI 镜像，常见的 `git`、`rg` 等本地工具可能不存在，技能里访问 `https://skillserver:3000` 也不会信任本地 CA。

这个仓库因此做了两件事：

1. 保留独立的 `hermes` 和 `dashboard` 容器，继续提供 gateway 与 dashboard。
2. 用 `Dockerfile.hermeswebui` 基于官方 Hermes WebUI 镜像做本地扩展，补齐当前仓库需要的 CLI 工具，并导入 SkillServer 的本地 CA，确保聊天里触发的 Hermes agent 工具链真的可用。
