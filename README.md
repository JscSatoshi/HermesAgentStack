...
- 对宿主机开放的端口有：Dashboard HTTPS `9119`、Hermes API `8642`、Hermes WebUI HTTPS `8787`。其中 API 默认仍只绑定本机回环；HTTPS 入口默认绑定 `0.0.0.0`，可同时通过 `localhost` 和当前局域网 IP 访问。
...
浏览器
    ...
    |    -> Hermes WebUI
    |
    `- https://<your-lan-ip>:8787
             -> https-proxy
             -> Hermes WebUI

宿主机直连 API：
    http://127.0.0.1:8642 -> Hermes API Server
```
...
Hermes WebUI
    -> Hermes Agent Runtime
         |- 模型调用 -> GitHub Copilot / GitHub Models
         `- 工具调用 -> Local terminal tools
                                        -> SkillServer (https://skillserver:3000)
                                             |- SearXNG
                                             `- Playwright Chromium
```
...
```text
这些资源由 Hermes WebUI 直接复用：
    - Hermes Home / Sessions / Memory
    - Hermes Agent 源码卷
    - /workspace
```
...
- Hermes Dashboard: `https://localhost:9119`
- Hermes WebUI: `https://localhost:8787`
- Hermes Dashboard（局域网）: `https://<your-lan-ip>:9119`
- Hermes WebUI（局域网）: `https://<your-lan-ip>:8787`
...
| `HERMES_BIND_HOST` | HTTPS 入口绑定地址；`127.0.0.1` 仅本机访问，`0.0.0.0` 允许本机和局域网访问 |
...
| `HERMES_TLS_COMMON_NAME` | HTTPS 证书通用名（CN），默认 `hermes-local` |
| `HERMES_TLS_EXTRA_DNS` | 追加到 HTTPS 证书 SAN 的 DNS，逗号分隔 |
| `HERMES_TLS_EXTRA_IPS` | 追加到 HTTPS 证书 SAN 的 IP，逗号分隔 |
...
- `local-https.crt` / `local-https.key`：CN 默认是 `hermes-local`，并签给 `skillserver`、`localhost`、`127.0.0.1` 以及当前主局域网 IP 的共享服务端证书
...
当前 HTTPS 入口默认由 Nginx 反代承接，证书会自动包含 `localhost`、`127.0.0.1` 和当前主局域网 IP，因此可以同时通过 `https://localhost:...` 和 `https://<your-lan-ip>:...` 访问。
